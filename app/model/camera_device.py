...
class CameraApp:
    def __init__(self, camera_type="RealSense", video_width=640, video_height=480, video_fps=30, video_duration_seconds=4):
        self.face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
        if self.face_cascade.empty():
            raise IOError("Failed to load haarcascade_frontalface_default.xml")

        self.camera = CameraDevice(camera_type=camera_type)
        self.video_recorder = VideoRecorder()
        self.video_recorder.set_config(width=video_width, height=video_height, fps=video_fps, duration_seconds=video_duration_seconds)
        self.start_ts = None
        self.is_running = True # Flag to control the main loop

    def _handle_quit_key(self):
        """Handles the 'q' key press to signal the application to stop."""
        self.is_running = False
        # No 'break' here, as we want the 'break' to happen in the run loop explicitly

    def _handle_toggle_recording_key(self, current_frame):
        """Handles the 's' key press to start or stop recording."""
        if not self.video_recorder.is_recording:
            print("Starting recording...")
            self.video_recorder.start('output.mp4', current_frame)
    ...
    def run(self):
        """Runs the main camera application loop."""
        while self.is_running:
            if self.camera.camera_type == "RealSense":
                if self.video_recorder.is_recording:
                    frame = self.camera.get_frame()
                else:
                    _, frame, _, have_face = self.camera.camera.show_frame(self.face_cascade)
            else:
                frame = self.camera.get_frame()
                have_face = False

            if frame is None:
                print("No frame received. Stopping application.")
                self.is_running = False # Signal to stop
                break # Immediately exit loop if no frame
            
            # If is_running became False from a previous key press (like 'q'),
            # we should also break here before further processing of the frame.
            if not self.is_running:
                break # Immediately exit loop if stop signal was given
            ...
            # cv2.imshow(f"{self.camera.camera_type} View", frame)
            ...
            if key == ord('q'):
                ...
                # If 'q' is pressed, we want to break immediately after handling it.
                # The _handle_quit_key only sets the flag, so we break here.
                # break # <--- IMMEDIATE BREAK AFTER 'Q'
            elif key == ord('s'):
                self._handle_toggle_recording_key(frame)

        # # The loop has ended (either 'q' was pressed, frame was None, or is_running became False)
        self.stop() # Call the stop method to clean up



    def stop(self):
        """Performs all necessary cleanup for the camera application."""
        print("Stopping CameraApp and cleaning up resources...")
        if self.video_recorder.is_recording:
            self.video_recorder.stop() # Ensure recording is stopped
        self.camera.stop() # Release camera resources
