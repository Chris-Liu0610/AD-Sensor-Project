...
class AppController:
    def __init__(self):
        ...
        self.camera_types = ["RealSense"]
        self.cam_idx = 0
        self.camera = CameraDevice(camera_type=self.camera_types[self.cam_idx])
        self.video_recorder = VideoRecorder()
        self.video_recorder.set_config(width=640, height=480, fps=30, duration_seconds=4)
        self.start_ts = None
    ...
    def handle_switch_camera(self):
        self.cam_idx = (self.cam_idx + 1) % len(self.camera_types)
        self.camera.switch_camera(self.camera_types[self.cam_idx])

    def handle_record_toggle(self, frame):
        if not self.video_recorder.is_recording:
            self.video_recorder.start("output.mp4", frame)
            self.start_ts = time.perf_counter()  # start stopwatch
        else:
            self.video_recorder.stop()
            self.start_ts = None  # stop stopwatch
    ...
    def run(self):
        while True:
            ...
            if key == ord('q'):
                ...
                self.handle_switch_camera()
            elif key == ord('s'):
                self.handle_record_toggle(frame)
