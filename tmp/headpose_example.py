...
class FacePoseDetector:
    ...
    def get_face_3d_model(self):
        ...
        face_3d_model = {
            ...
            'chin': [0.0, -7.5, -3.0],
            'left_eye_corner': [-3.5, 2.5, -3.0],
            'right_eye_corner': [3.5, 2.5, -3.0],
            'left_mouth_corner': [-2.5, -2.5, -2.0],
            'right_mouth_corner': [2.5, -2.5, -2.0],
            'forehead': [0.0, 5.0, -3.0]
        }
    ...
    def draw_rotation_arrows(self, image, rvec, tvec, nose_tip_2d, arrow_length=80):
        ...
        # 獲取歐拉角
        pitch, yaw, roll = self.get_euler_angles(rvec)
        ...
        # Roll 箭頭呈現為一個傾斜的箭頭
        roll_length = arrow_length * 0.7
        roll_end_x = int(nose_tip_2d[0] + roll_length * math.cos(roll_angle - math.pi/2))
        roll_end_y = int(nose_tip_2d[1] + roll_length * math.sin(roll_angle - math.pi/2))
        
        cv2.arrowedLine(
            image,
            (int(nose_tip_2d[0]), int(nose_tip_2d[1])),
            (roll_end_x, roll_end_y),
            (255, 0, 0),  # 藍色
            3,
            tipLength=0.3
        )
    ...
    def annotate_image(self, image, rvec, tvec, nose_tip_2d):
        ...
        # 在圖像上顯示角度信息
        info_text = [
            f"Pitch: {pitch:.1f} degree (Red)",
            f"Yaw: {yaw:.1f} degree (Green)",
            f"Roll: {roll:.1f} degree (Blue)"
        ]
...
def process_image(image_path):
    ...
    # 檢測人臉姿態
    rvec, tvec, nose_tip_2d = detector.detect_face_pose(image)
    
    if rvec is not None:
        # 標註圖像
        annotated_image = detector.annotate_image(image, rvec, tvec, nose_tip_2d)
        
        # 顯示結果
        cv2.imshow('Face Pose Detection - Triple Arrows', annotated_image)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
        
        # 保存結果
        cv2.imwrite('face_pose_triple_arrows.jpg', annotated_image)
    else:
        print("未檢測到人臉")
